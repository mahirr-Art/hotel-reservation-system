    ...
    <div>
      ...
      <section className="bg-teal-700 text-white">
        <div className="max-w-5xl mx-auto px-6 pt-20 pb-24 text-center">
          <p className="uppercase tracking-widest text-xs text-teal-100 mb-4">Hoş geldiniz</p>
          <h1 className="text-4xl md:text-5xl font-semibold leading-tight mb-6">
            Şehrin kalbinde huzurlu bir mola
          </h1>
          <form action="/rezervasyon" className="bg-white rounded-2xl p-4 grid gap-3 sm:grid-cols-5 max-w-3xl mx-auto text-neutral-900">
            <select name="city" className="rounded-lg border px-3 py-2 text-sm">
            ...
            <button type="submit" className="rounded-lg bg-teal-600 text-white px-4 py-2 text-sm font-medium hover:bg-teal-700">Ara</button>
          </form>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        <h2 className="text-2xl font-semibold mb-8 text-teal-900">Öne Çıkan Odalar</h2>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {featuredRooms.map((room) => (
              <Link key={room.id} href={`/odalarimiz/${room.id}`} className="rounded-2xl border overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <div className="p-5">
                  <p className="text-xs uppercase text-teal-600 mb-1">{room.category.name}</p>
      ...
      <section className="bg-teal-50 py-16">
        <div className="max-w-5xl mx-auto px-6">
          <h2 className="text-2xl font-semibold mb-8 text-center text-teal-900">Misafirlerimiz Ne Diyor</h2>
          <div className="grid gap-6 md:grid-cols-2">
            {testimonials.map((t) => (
              <div key={t.id} className="bg-white rounded-2xl border p-6">
                ...
                <p className="text-sm text-teal-600">— {t.authorTag}</p>
              </div>
            ))}
